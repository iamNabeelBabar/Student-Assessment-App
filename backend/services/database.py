...
async def create_assessment(
    ...
    """
    Upsert the assessment for a topic.
    Uses upsert on topic_id so that if a placeholder row was already created
    by update_assessment_status("generating"), we UPDATE it instead of
    hitting a 409 duplicate key conflict.
    """
    db = get_supabase()
    ...
    if existing:
        # Placeholder row exists — update it with the real questions
        result = (
...
async def update_assessment_status(
    ...
    """
    Update assessment generation_status for a topic.
    - If a completed assessment already exists, never overwrite it with 'failed'.
    - Creates a placeholder row on first call (pending → generating) so
      the frontend can poll before questions are ready.
    """
    db = get_supabase()
    ...
    if existing:
        # Never downgrade a completed assessment to failed
        if existing.get("generation_status") == "completed" and status == "failed":
            return existing
        ...
        # Create placeholder row so frontend can poll status
        data = {
            "topic_id":          topic_id,
            "mcq_questions":     [],
            "short_questions":   [],
